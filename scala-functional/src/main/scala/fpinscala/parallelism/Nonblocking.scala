...
object nonblocking_test {
  ...
  def assertParEqual[T](es: ExecutorService)(p: Par[T], p2: Par[T], passMsg: String=""): Unit = {
    val expected = run(es)(p)
    val actual = run(es)(p2)
    val message = s"\n expected: $expected\n actual  : $actual"
    assert(expected == actual, message)
    println("PASS! "+passMsg)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    assertParEqual(es)(choiceN(unit(1))(unit_list(List("영", "일"))), unit("일"))
    assertParEqual(es)(choice(unit(true))(unit("참"), unit("거짓")), unit("참"))
    val map = Map(1->"일", 2->"이", 3->"삼")
    assertParEqual(es)(chooser(unit(3))(n => unit(map.getOrElse(n, "없음"))), unit("삼"))
    es.shutdown()
    println("shutdown")
  }
}
