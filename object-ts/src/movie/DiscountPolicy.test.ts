import { AmountDiscountPolicy, NoneDiscountPolicy, PercentDiscountPolicy } from './DiscountPolicy';
import { NeverDiscountCondition, AlwaysDiscountCondition } from './DiscountCondition';
...
describe('DiscountPolicy', () => {
  describe('AmountDiscountPolicy', () => {
    test('할인액은 일정하다.', () => {
      const policy = new AmountDiscountPolicy(Money.wons(5000), [AlwaysDiscountCondition]);
  ...
  describe('PercentDiscountPolicy', () => {
    test('percent로 0.5를 넣으면, 50% 할인이 된다', () => {
      const policy = new PercentDiscountPolicy(0.5, [AlwaysDiscountCondition]);
